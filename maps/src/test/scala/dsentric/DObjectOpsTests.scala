...
case class Custom(value:Map[String, Any]) extends DObject with DObjectOps[Custom] {
  protected def wrap(value: Map[String, Any]): Custom =
    Custom(value)
}

case class CustomParams(id:UUID)(val value:Map[String, Any]) extends DObject with DObjectOps[CustomParams] {
  protected def wrap(value: Map[String, Any]): CustomParams =
    CustomParams(id)(value)
}

class DObjectOpsTests extends AnyFunSuite with Matchers {
...
//      CustomContract.nested.$forceDrop
  ...
  implicit val customCodec =
    DefaultCodecs.dObjectOpsCodec[Custom](Custom)
