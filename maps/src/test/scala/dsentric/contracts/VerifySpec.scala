  ...
  describe("Value Property behaviour") {
    ...
    describe("Contract Codec Type") {
      ...
      it("Should return failure if expected object missing is empty, even if empty satisfies contract") {
        val base = DObject.empty
        UnusualContractCodec.$verify(base).left.value should contain (ExpectedFailure(UnusualContractCodec, Path("property")))
      ...
      it("Should return failure if maybe object does not satisfy contract conditions") {
        val base = DObject("property" ::= ("maybe" := false))
        MaybeContractCodec.$verify(base).left.value should contain (ExpectedFailure(MaybeContractCodec, Path("property", "expected")))
      }
    ...
    describe("Map Codec Type") {
      ...
      it("Should return incorrect type failures for null and empty objects") {
        ...
        MapCodec.$verify(base).left.value should contain (IncorrectTypeFailure(MapCodec, Path("property", "key1"), intCodec, DNull))
        val base2 = DObject("property" ::= ("key1" := DObject.empty))
        MapCodec.$verify(base2).left.value should contain (IncorrectTypeFailure(MapCodec, Path("property", "key1"), intCodec, RawObject.empty))
      }
      it("Should return failure with contract failures") {
        val base = DObject("property" ::= ("key3" := DObject("maybe" := false)))
        MaybeMapCodec.$verify(base).left.value should contain (ExpectedFailure(MaybeMapCodec, Path("property", "key3", "expected")))
      }
    ...
    describe("Collection Codec Type") {
      ...
      it("Should return failure with contract failures") {
        ...
        MaybeCollectionCodec.$verify(base).left.value should contain (ExpectedFailure(MaybeCollectionCodec, Path("property", 0, "expected")))
      }
    ...
    describe("Type Contract Codec Type") {
      ...
      it("Should return failure if target contract not satisfied") {
        ...
        TypeContractCodec.$verify(base).left.value should contain (IncorrectTypeFailure(TypeContractCodec, Path("property", "property1"), intCodec, "value"))
        val base2 = DObject("property" := DObject("type" := "type1", "property2" := 1234))
        TypeContractCodec.$verify(base2).left.value should contain (ExpectedFailure(TypeContractCodec, Path("property", "property1")))
      }
      ...
      it("Should return resolution failure if expected object is empty") {
        val base = DObject("property" := DObject.empty)
        TypeContractCodec.$verify(base).left.value should contain (ContractTypeResolutionFailure(TypeContractCodec, Path("property"), RawObject.empty))
      }
  ...
  describe("Object Property behaviour") {
    ...
    describe("Expected property") {
      ...
      it("Should fail on incorrect type if expected property is null") {
        val base = DObject("property" := DNull)
        ExpectedObject.$verify(base).left.value should contain (
          IncorrectTypeFailure(ExpectedObject.property, DNull)
        )
      }
      ...
      it("Should return object if contains null and empty object property values") {
        val base = DObject("property" := DObject("expected" := "value", "additional1" := DNull, "additional" := DObject.empty))
        ExpectedObject.$verify(base).value.validObject shouldBe base
      }
    ...
    describe("Maybe property") {
      ...
      it("Should return failure if maybe object is empty and has expected property ") {
        val base = DObject("property" := DObject.empty)
        MaybeObject.$verify(base).left.value should contain (
      ...
      it("Should return incorrect type if maybe object is null ") {
        val base = DObject("property" := DNull)
        MaybeObject.$verify(base).left.value should contain (
          IncorrectTypeFailure(MaybeObject.property, DNull)
        )
      }
      ...
      it("Should return object if contains null and empty object property values") {
        val base = DObject("property" := DObject("expected" := "value", "additional2" := DNull, "additional" := DObject.empty))
        MaybeObject.$verify(base).value.validObject shouldBe base
      }
    }
  }
  ...
  describe("Additional Property behaviour") {
    ...
    describe("Closed object") {
      ...
      it("Should fail if object contains additional properties") {
        ...
        Closed.$verify(base).left.value should contain (
    ...
    describe("Open object") {
      ...
      it("Should fail if nested contract fails to validate") {
        ...
        Additional.$verify(base).left.value should contain (ExpectedFailure(Additional, Path("codecProperty", "obj1", "expected")))
      }
    }
  }
}
