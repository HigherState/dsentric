...
import dsentric.failure.{AdditionalElementFailure, ClosedContractFailure, ContractTypeResolutionFailure, CoproductTypeValueFailure, ExpectedFailure, Failure, IncorrectKeyTypeFailure, IncorrectTypeFailure, MissingElementFailure, ValidResult}
...
private[contracts] trait GetOps {
  ...
  protected def getMap[D <: DObject, C, K, V](
    ...
    raw.view
      ...
      case Left(_) if badTypes == DropBadTypes =>
        NotFound
      case Left(lb)                            =>
        val head :: tail = lb.result(): @unchecked
        Failed(head, tail)
    }
  }
  ...
  protected def getParameterisedContract[D <: DObject, D2 <: DObject](
    contract: ContractLike[D],
    path: Path,
    badTypes: BadTypes,
    setDefaultValues: Boolean,
    codec: DParameterisedContractCodec[D2],
    raw: RawObject
  ): Available[D2] = {
  ...
  protected def getProduct[D <: DObject, T, E <: Tuple, H <: Tuple](
    contract: ContractLike[D],
    path: Path,
    badTypes: BadTypes,
    setDefaultValues: Boolean,
    ...
    raw.zipWithIndex
      ...
      .foldRight[Either[ListBuffer[Failure], Tuple]](init) {
   ...
   * @return
   */
  protected def getCoproduct[D <: DObject, T, H <: Tuple](
    contract: ContractLike[D],
    path: Path,
    badTypes: BadTypes,
    setDefaultValues: Boolean,
    ...
    codec.codecsList.foldLeft[Available[T]](NotFound) {
      ...
      case (a, c)           =>
        getCodec(contract, path, badTypes, setDefaultValues)(c -> raw) match {
          case Found[c.Out @unchecked](t) => codec.lift[c.Out](t, c).fold[Available[T]](NotFound)(Found(_))
