...
private[dsentric] trait ReduceOps {
  ...
  protected def reduceContract[D <: DObject, D2 <: DObject](contract:ContractFor[D], path:Path, badTypes:BadTypes, codecContract:ContractFor[D2], raw:RawObject):Available[RawObject] = {
  ...
  protected def reduceTypeContract[D <: DObject, D2 <: DObject](contract:ContractFor[D], path:Path, badTypes:BadTypes, typeCodecs:PartialFunction[D2, ContractFor[D2]], d2Cstr: RawObject => D2, raw:RawObject):Available[RawObject] =
...
private[dsentric] object ReduceOps extends ReduceOps