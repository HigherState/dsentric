...
class AspectFor[D <: DObject, D2 <: D](_source: ContractFor[D])(
  f: PartialFunction[Property[D, ?], Option[AspectProperty[D2, ?]]] = PartialFunction.empty
) extends ContractLike[D2] with reflect.Selectable {
  ...
  //TODO support pulling from property if has withAdditional
  def \\\?[Key, Value](property: ObjectProperty[D], merged: ObjectProperty[D]*)(
    f: PartialFunction[Property[D, ?], Option[AspectProperty[D2, ?]]]
  ...
  private[contracts] var __fields: Map[String, Property[D2, ?]] = uninitialized
  @volatile
  private var _bitmap0: Boolean                                 = false

  final def _fields: Map[String, Property[D2, ?]] =
      ...
      this.synchronized {
        ...
        __fields = aspectFields ++
          this.getClass.getMethods.flatMap { m =>
            if (
              classOf[Property[D2, ?]].isAssignableFrom(
                m.getReturnType
...
class Aspect[D <: DObject](_source: ContractFor[D])(
  f: PartialFunction[Property[D, ?], Option[AspectProperty[D, ?]]] = PartialFunction.empty
) extends AspectFor[D, D](_source)(f)

object Aspect {

  def apply[D <: DObject](_root: ContractFor[D])(
    f: PartialFunction[Property[D, ?], Option[AspectProperty[D, ?]]]
  ): Aspect[D] =
    new Aspect[D](_root)(f)

  private[contracts] def getFields[D <: DObject, D2 <: D](
    contract: BaseContract[D]
  )(f: PartialFunction[Property[D, ?], Option[AspectProperty[D2, ?]]]): Map[String, AspectProperty[D2, ?]] = {

    def reparent[B <: BaseContract[D2]](b: B, fields: Map[String, AspectProperty[D2, ?]]): B = {
    ...
    def transformFields(fields: Map[String, Property[D, ?]]): Map[String, AspectProperty[D2, ?]] =
      fields.flatMap { pair =>
...
trait DAspectSyntax {
  ...
  type AspectPropertyFunction[D <: DObject, D2 <: DObject] =
    PartialFunction[Property[D, ?], Option[AspectProperty[D2, ?]]]

  implicit def toPropertyAspectOps[D <: DObject](p: Property[D, ?]): PropertyAspectOps[D] =
...
final class PropertyAspectOps[D <: DObject](val p: Property[D, ?]) extends AnyVal {
  def $asExpected[D2 <: D](): AspectProperty[D2, ?] =
    p match {
      case o: ObjectProperty[D]   =>
        val dataOperators =
          o._dataOperators
            .collect { case d: (DataOperator[DObject] & Expected) => d }
        new ObjectPropertyAspectOps[D](o).$asExpected[D2](dataOperators)
      case v: ValueProperty[D, ?] =>
        val dataOperators =
          v._dataOperators
            .collect { case d: (DataOperator[?] & Expected) => d }
        new ValuePropertyAspectOps[D, Any](v.asInstanceOf[ValueProperty[D, Any]]).$asExpected[D2](dataOperators)
      case _                      =>
        ???
    }

  def $asMaybe[D2 <: D](): AspectProperty[D2, ?] =
    p match {
      case o: ObjectProperty[D]   =>
        val dataOperators =
          o._dataOperators
            .collect { case d: (DataOperator[DObject] & Optional) => d }
        new ObjectPropertyAspectOps[D](o).$asMaybe[D2](dataOperators)
      case v: ValueProperty[D, ?] =>
        val dataOperators =
          v._dataOperators
            .collect { case d: (DataOperator[?] & Optional) => d }
        new ValuePropertyAspectOps[D, Any](v.asInstanceOf[ValueProperty[D, Any]]).$asMaybe[D2](dataOperators)
      case _                      =>
        ???
    }

  def $asExpected[D2 <: D](replaceDataOperators: List[DataOperator[DObject] & Expected]): AspectProperty[D2, ?] =
    p match {
      case o: ObjectProperty[D]   =>
  ...
  def $asMaybe[D2 <: D](replaceDataOperators: List[DataOperator[DObject] & Optional]): AspectProperty[D2, ?] =
    p match {
      case o: ObjectProperty[D]   =>
...
final class ObjectPropertyAspectOps[D <: DObject](val p: ObjectProperty[D]) extends AnyVal {
  ...
  def $asExpected[D2 <: D](
    replaceDataOperators: List[DataOperator[DObject] & Expected]
  ): ExpectedObjectAspectProperty[D2] =
    ExpectedObjectAspectProperty[D2](p, replaceDataOperators)

  def $asExpected[D2 <: D](
    appendDataOperator: DataOperator[DObject] & Expected,
    tail: DataOperator[DObject] & Expected*
  ): ExpectedObjectAspectProperty[D2] = {
    val dataOperators =
      p._dataOperators
        .collect { case d: (DataOperator[DObject] & Expected) => d } ::: appendDataOperator :: tail.toList
    ExpectedObjectAspectProperty[D2](p, dataOperators)
  }
  ...
  def $asMaybe[D2 <: D](
    replaceDataOperators: List[DataOperator[DObject] & Optional] = Nil
  ): MaybeObjectAspectProperty[D2] =
    MaybeObjectAspectProperty[D2](p, replaceDataOperators)

  def $asMaybe[D2 <: D](
    appendDataOperator: DataOperator[DObject] & Optional,
    tail: DataOperator[DObject] & Optional*
  ): MaybeObjectAspectProperty[D2] = {
    val dataOperators =
      p._dataOperators
        .collect { case d: (DataOperator[DObject] & Optional) => d } ::: appendDataOperator :: tail.toList
    MaybeObjectAspectProperty[D2](p, dataOperators)
  }
...
final class ValuePropertyAspectOps[D <: DObject, T](val p: ValueProperty[D, T]) extends AnyVal {
  ...
  def $asExpected[D2 <: D](): ExpectedAspectProperty[D2, T] = {
    val dataOperators =
      p._dataOperators
        .collect { case d: (DataOperator[T] & Expected) => d }
    ExpectedAspectProperty(p._key, p._codec, dataOperators)
  }

  def $asExpected[D2 <: D](replaceDataOperators: List[DataOperator[T] & Expected]): ExpectedAspectProperty[D2, T] =
    ExpectedAspectProperty(p._key, p._codec, replaceDataOperators)

  def $asExpected[D2 <: D](
    appendDataOperators: DataOperator[T] & Expected,
    tail: DataOperator[T] & Expected*
  ): ExpectedAspectProperty[D2, T] = {
    val dataOperators =
      p._dataOperators
        .collect { case d: (DataOperator[T] & Expected) => d } ::: appendDataOperators :: tail.toList
    ExpectedAspectProperty(p._key, p._codec, dataOperators)
  }
  ...
  def $asMaybe[D2 <: D](): MaybeAspectProperty[D2, T] = {
    val dataOperators =
      p._dataOperators
        .collect { case d: (DataOperator[T] & Optional) => d }
    MaybeAspectProperty(p._key, p._codec, dataOperators)
  }

  def $asMaybe[D2 <: D](replaceDataOperators: List[DataOperator[T] & Optional] = Nil): MaybeAspectProperty[D2, T] =
    MaybeAspectProperty(p._key, p._codec, replaceDataOperators)

  def $asMaybe[D2 <: D](
    appendDataOperators: DataOperator[T] & Optional,
    tail: DataOperator[T] & Optional*
  ): MaybeAspectProperty[D2, T] = {
    val dataOperators =
      p._dataOperators
        .collect { case d: (DataOperator[T] & Optional) => d } ::: appendDataOperators :: tail.toList
    MaybeAspectProperty(p._key, p._codec, dataOperators)
  }
  ...
  def $asDefault[D2 <: D](
    default: T,
    replaceDataOperators: List[DataOperator[T] & Optional]
...
final class ExpectedPropertyAspectOps[D <: DObject, T](val p: ExpectedProperty[D, T]) extends AnyVal {
  def $appendDataOperators[D2 <: D](dataOperators: DataOperator[T] & Expected*): ExpectedAspectProperty[D2, T] =
    ExpectedAspectProperty(p._key, p._codec, p._dataOperators ++ dataOperators)

  def $replaceDataOperators[D2 <: D](dataOperators: DataOperator[T] & Expected*): ExpectedAspectProperty[D2, T] =
...
final class MaybePropertyAspectOps[D <: DObject, T](val p: MaybeProperty[D, T]) extends AnyVal {
  def $appendDataOperators[D2 <: D](dataOperators: DataOperator[T] & Optional*): MaybeAspectProperty[D2, T] =
    MaybeAspectProperty(p._key, p._codec, p._dataOperators ++ dataOperators)

  def $replaceDataOperators[D2 <: D](dataOperators: DataOperator[T] & Optional*): MaybeAspectProperty[D2, T] =
...
final class DefaultPropertyAspectOps[D <: DObject, T](val p: DefaultProperty[D, T]) extends AnyVal {
  ...
  def $appendDataOperators[D2 <: D](dataOperators: DataOperator[T] & Optional*): DefaultAspectProperty[D2, T] =
    DefaultAspectProperty(p._key, p._default, p._codec, p._dataOperators ++ dataOperators)

  def $replaceDataOperators[D2 <: D](dataOperators: DataOperator[T] & Optional*): DefaultAspectProperty[D2, T] =
...
final class ExpectedObjectAspectOps[D <: DObject](val p: ExpectedObjectProperty[D]) extends AnyVal {
  ...
  def $appendDataOperators[D2 <: D](
    dataOperators: DataOperator[DObject] & Expected*
  ): ExpectedObjectAspectProperty[D2] =
...
final class MaybeObjectAspectOps[D <: DObject](val p: MaybeObjectProperty[D]) extends AnyVal {
  def $appendDataOperators[D2 <: D](
    dataOperators: DataOperator[DObject] & Optional*
  ): MaybeObjectAspectProperty[D2] =
    MaybeObjectAspectProperty(p, p._dataOperators ++ dataOperators)

  def $replaceDataOperators[D2 <: D](dataOperators: DataOperator[DObject] & Optional*): MaybeObjectAspectProperty[D2] =
