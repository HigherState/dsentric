 ...
 * TODO - tail recursion would be good here.
 */
trait DValueOps {
...
trait DataOps {
  ...
  def nestedExists[D <: DObjectOps[D]](value: DObjectOps[D])(f: (String, Data) => Boolean): Boolean =
    value.exists(p => nestedKeyValueExists(p._1, p._2.value)(f))
  ...
  private[dsentric] def nestedValueMap[T, U](value: Raw, pf: PartialFunction[T, U])(implicit
    D1: DCodec[T],
    D2: DCodec[U]
  ): Raw =
    value match {
  ...
  private[dsentric] def nestedKeyValueMap[T, U](
    value: Any,
    pf: PartialFunction[(String, T), Option[(String, U)]]
  )(implicit D1: DCodec[T], D2: DCodec[U]): Raw =
    value match {
      case m: RawObject @unchecked =>
        m.flatMap { kv =>
  ...
  private[dsentric] def nestedKeyMap(value: Any, pf: PartialFunction[String, Option[String]]): Raw =
    value match {
      case m: RawObject @unchecked =>
        m.flatMap { kv =>
          pf.lift(kv._1)
            .fold[Option[(String, Any)]](Some(kv._1 -> nestedKeyMap(kv._2, pf)))(v =>
              v.map(nk => nk -> nestedKeyMap(kv._2, pf))
            )
        }
      case v: RawArray @unchecked  =>
  ...
  private[dsentric] def nestedKeyValueExists(key: String, value: Raw)(f: (String, Data) => Boolean): Boolean =
    value match {
      case m: RawObject @unchecked =>
        if (f(key, ForceWrapper.dObject(m))) true
        else
          m.exists(p => nestedKeyValueExists(p._1, p._2)(f))
      case v: RawArray @unchecked  =>
        f(key, ForceWrapper.dArray(v))
      case v                       =>
        f(key, ForceWrapper.dValue(v))
