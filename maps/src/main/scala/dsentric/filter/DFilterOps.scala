...
trait DFilterOps {
  ...
  private[dsentric] def apply(value: Option[Any], query: Map[String, Any], notFoundAsNull: Boolean): Boolean =
    query.forall {
      ...
      case ("$regex" | "$options", _)                     =>
        query
          .get("$regex")
          .collect { case v: String =>
            val _options = query.get("$options").collect { case o: String => s"(?$o)" }.getOrElse("")
            value.collect { case s: String => (_options + v).r.pattern.matcher(s).matches }.getOrElse(false)
          }
          .getOrElse(false)
      case ("$like", v: String)                           =>
        value
      ...
      case ("$elemMatch", v: Map[String, Any] @unchecked) =>
        value
          .collect { case seq: RawArray @unchecked => seq.exists(s => apply(Some(s), v, notFoundAsNull)) }
          .getOrElse(false)
      case ("$elemMatch", v)                              =>
  ...
  private val order: ((Raw, Raw)) => Option[Int] = DValueOps.order.lift
