...
object Definition {
  ...
  def contractObjectDefinitionRef[D <: DObject](contract:ContractFor[D], infos: Infos, defs:Definitions, forceNested:Boolean = false):(String, Infos, Definitions) = {
    ...
    defs.find(_.definition.contains(name))
      ...
      .getOrElse{
        ...
        val (c, infos2, d) = baseContractObjectDefinition(contract._fields, contract, contractInfo, infos1, Vector.empty, false)
        (name, infos2, d :+ c)
      }
  }
  ...
  private def baseContractObjectDefinition[D <: DObject](fields:Map[String, Property[D, _]], contract:BaseContract[D], contractInfo:ContractInfo, infos:Infos, defs:Definitions, forceNested:Boolean):(ObjectDefinition, Infos, Definitions) = {
      ...
      .foldLeft((Set.empty[PropertyDefinition], infos, defs)) {
        ...
        case ((p, infos0, defs0), (name, b:BaseContract[D]@unchecked with Property[D ,_], schema)) =>
          ...
          if (bInfo.inherits.size == 1 && bInfo.fields.isEmpty && b._dataOperators.isEmpty) {
            val (ref, infos2, defs1) = baseContractObjectDefinition(b._fields, b, bInfo.inherits.head, infos1, defs0, false)
  ...
  private def inheritFold[D <: DObject](fields:Map[String, Property[D, _]], contract:BaseContract[D], inherits:Vector[ContractInfo], infos:Vector[ContractInfo], defs:Vector[ObjectDefinition], forceNested:Boolean): (Vector[String], Vector[ContractInfo], Vector[ObjectDefinition]) =
