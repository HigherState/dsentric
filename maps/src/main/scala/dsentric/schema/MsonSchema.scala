...
class MsonSchema(tab: String, newLine: String) {
  ...
  def convertObjectDatastructures(objectDefinitions: (ObjectDefinition, Definition.Definitions)): String = {
    ...
    combined.foreach { o =>
      sb.append(
        s"## ${escape(o.definition.getOrElse(throw SchemaGenerationException("Object definition required")))} (object)"
      )
  ...
  private def objectStructure(stringBuilder: StringBuilder, objectDefinition: ObjectDefinition): Unit = {
    stringBuilder.append(
      s"# ${escape(objectDefinition.definition.getOrElse(throw SchemaGenerationException("Object definition required")))} (object)"
    )
  ...
  private def convertPropertyDefinition(
    ...
    newTypeDefinition match {
      ...
      case _: ArrayDefinition =>
        ...
        propertyDefinition.default
          .collect { case s: Iterable[Any] => s.map(i => escape(i.toString)).mkString(", ") }
          .foreach { p =>
            stringBuilder.append(newLine)
            stringBuilder.append(tabs)
            stringBuilder.append(tab)
