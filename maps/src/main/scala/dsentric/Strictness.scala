...
trait Strictness {
  def apply[T](value:Map[String, Any], path:Path, codec:JCodec[T]):Option[Option[T]] =
    PathOps.traverse(value, path).fold[Option[Option[T]]](Some(None)){apply(_, codec)}

  def apply[T](value:Any, codec:JCodec[T]):Option[Option[T]]
}
