...
object DParameter                   {
  ...
  implicit def rawObjectValueParameter[K <: String, T <: Tuple](implicit
 ...
 * @tparam H
 */
final case class DParameterisedContractCodec[D <: DObject](contract: ContractLike[D])(implicit
  generic: Mirror.ProductOf[D],
  label: LabelledType[D],
  hCodec: => DParameter[label.Out],
) extends DCodec[D] {
  def unapply(raw: Raw): Option[D] =
    raw match {
      case rawObject: RawObject @unchecked =>
        val (r2: RawObject, tail) = hCodec.decode(rawObject)
        tail.map(f => generic.fromProduct(f(r2))).toOption
      case _                               =>
        None
    }
  ...
  def unapply(a: Raw): Option[T] =
    a match {
      case rawArray: RawArray if rawArray.size == codecsArray.length =>
        rawArray
          .zip(codecsArray)
          .foldRight[Option[Tuple]](Some(EmptyTuple)) { (p, h) =>
            h.flatMap { hlist =>
