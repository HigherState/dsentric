...
import dsentric.failure.{ClosedContractFailure, ContractTypeResolutionFailure, CoproductTypeValueFailure, ExpectedFailure, Failure, IncorrectKeyTypeFailure, IncorrectTypeFailure, ValidResult}
...
private[contracts] trait DeltaReduceOps extends ReduceOps {
  ...
  protected def deltaReduceParameterisedContract[D <: DObject, D2 <: DObject](
    contract: ContractLike[D],
    path: Path,
    badTypes: BadTypes,
    ...
    current match {
      case currentObject: RawObject @unchecked if currentObject.nonEmpty =>
        ...
        codec.contract.__reduceDelta(
          ...
          case (d: DeltaReduced[RawObject] @unchecked, Nil)                  =>
  ...
  protected def deltaReduceCoproduct[D <: DObject, T, H <: Tuple](
    contract: ContractLike[D],
    path: Path,
    badTypes: BadTypes,
